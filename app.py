...
def init_db():
    ...
    if db.execute("SELECT COUNT(*) AS total FROM users WHERE role = 'admin'").fetchone()["total"] == 0:
        admin_email = os.environ.get("ADMIN_EMAIL", "admin@roof.local")
        admin_password = os.environ.get("ADMIN_PASSWORD", "admin12345")
        if app.config["USE_POSTGRES"]:
            db.execute("INSERT INTO users (name, email, password_hash, role) VALUES (%s, %s, %s, 'admin') ON CONFLICT (email) DO NOTHING", ("Roof Admin", admin_email, generate_password_hash(admin_password)))
        else:
            db.execute("INSERT OR IGNORE INTO users (name, email, password_hash, role) VALUES (?, ?, ?, 'admin')", ("Roof Admin", admin_email, generate_password_hash(admin_password)))
    db.commit()
...
def seed_content(db):
    ...
    for article in content["articles"]:
        db.execute("INSERT INTO articles (slug, category, title, excerpt, time, tone, author, body) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (article["id"], article["category"], article["title"], article["excerpt"], article.get("time", "5 phút"), article.get("tone", "moss"), article["author"], json.dumps(article.get("sections", []), ensure_ascii=False)))
...
def google_callback():
    ...
    if user is None:
        user_id = execute_insert_user("INSERT INTO users (name, email, google_id) VALUES (?, ?, ?)", (profile.get("name", "Roof member"), profile["email"].lower(), profile["sub"]))
    else:
        user_id = user["id"]
